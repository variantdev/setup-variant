...
let ownerRepo: string = 'mumoshu/variant';
let toolName: string = 'variant';
...
export async function getVariant(version: string) {
  ...
  if (!toolPath) {
    ...
    toolPath = await acquireVariant(version);
...
async function acquireVariant(version: string): Promise<string> {
  //
...
function getFileName(version: string): string {
  ...
  const filename: string = util.format(
    'download/v%s/variant_%s_%s_%s.%s',
    version,
    version,
    platform,
    arch,
